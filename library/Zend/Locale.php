...
class Zend_Locale
{
     ...
     * @return void
     */
    public static function setZAP($locale, $quality = 1)
    {
        ...
        self::$_auto = self::getBrowser() + self::getEnvironment() + self::getZAP();
    }
     ...
     * ordered by quality
     *
     * Standard Searchorder is Browser, Environment, ZAP
     *
    ...
    public static function getOrder($order = null)
    {
        switch ($order) {
            case self::ENVIRONMENT:
                self::$_breakChain = true;
                $languages         = self::getEnvironment() + self::getBrowser() + self::getZAP();
                break;

            case self::ZFDEFAULT:
                self::$_breakChain = true;
                $languages         = self::getZAP() + self::getEnvironment() + self::getBrowser();
                break;

            default:
                self::$_breakChain = true;
                $languages         = self::getBrowser() + self::getEnvironment() + self::getZAP();
                break;
        }
