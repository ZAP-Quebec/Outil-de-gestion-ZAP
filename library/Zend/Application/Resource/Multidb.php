...
class Zend_Application_Resource_Multidb extends Zend_Application_Resource_ResourceAbstract
{
    ...
    public function init()
    {
        ...
        foreach ($options as $id => $params) {
            ...
            $default = (int) (
                isset($params['isZAPTableAdapter']) && $params['isZAPTableAdapter']
                || isset($params['default']) && $params['default']
            );
            unset(
                $params['adapter'],
                $params['default'],
                $params['isZAPTableAdapter']
            );

            $this->_dbs[$id] = Zend_Db::factory($adapter, $params);

            if ($default) {
                $this->_setZAP($this->_dbs[$id]);
            }
        }
     ...
     */
    public function getZAPDb($justPickOne = true)
    {
     ...
     */
    protected function _setZAP(Zend_Db_Adapter_Abstract $adapter)
    {
        Zend_Db_Table::setZAPAdapter($adapter);
