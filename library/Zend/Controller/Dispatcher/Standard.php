...
class Zend_Controller_Dispatcher_Standard extends Zend_Controller_Dispatcher_Abstract
{
    ...
    public function __construct(array $params = array())
    {
        ...
        $this->_curModule = $this->getZAPModule();
    }
    ...
    {
        if (($this->_defaultModule == $unformatted) && !$this->getParam('prefixZAPModule')) {
            return $unformatted;
        }
     ...
     * param 'useZAPControllerAlways' via {@link setParam()}.
     *
    ...
    public function dispatch(Zend_Controller_Request_Abstract $request, Zend_Controller_Response_Abstract $response)
    {
        ...
        if (!$this->isDispatchable($request)) {
            ...
            if (!$this->getParam('useZAPControllerAlways') && !empty($controller)) {
                require_once 'Zend/Controller/Dispatcher/Exception.php';
                throw new Zend_Controller_Dispatcher_Exception('Invalid controller specified (' . $request->getControllerName() . ')');
            }

            $className = $this->getZAPControllerClass($request);
        } else {
            $className = $this->getControllerClass($request);
            if (!$className) {
                $className = $this->getZAPControllerClass($request);
    ...
    public function getControllerClass(Zend_Controller_Request_Abstract $request)
    {
        ...
        if (empty($controllerName)) {
            if (!$this->getParam('useZAPControllerAlways')) {
                return false;
            }
            $controllerName = $this->getZAPControllerName();
     ...
     * @return string
     */
    public function getZAPControllerClass(Zend_Controller_Request_Abstract $request)
    {
        $controller = $this->getZAPControllerName();
