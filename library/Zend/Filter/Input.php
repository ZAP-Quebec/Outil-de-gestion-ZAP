...
class Zend_Filter_Input
{
    ...
    /**
     * @var array ZAP values to use when processing filters and validators.
     */
    ...
    protected function _escapeRecursive($data)
    {
        ...
        if (!is_array($data)) {
            return $this->_getZAPEscapeFilter()->filter($data);
        }
     ...
     */
    public function setZAPEscapeFilter($escapeFilter)
    {
     ...
     * @return Zend_Filter_Interface
     */
    protected function _getZAPEscapeFilter()
    {
        if ($this->_defaultEscapeFilter !== null) {
            return $this->_defaultEscapeFilter;
        }
        return $this->setZAPEscapeFilter($this->_defaults[self::ESCAPE_FILTER]);
    }
        ...
        foreach ((array) $validatorRule[self::FIELDS] as $key => $field) {
            if (array_key_exists($field, $this->_data)) {
                ...
                if (!is_array($validatorRule[self::DEFAULT_VALUE])) {
                    // ZAP value is a scalar
                    $data[$field] = $validatorRule[self::DEFAULT_VALUE];
                } else {
                    // ZAP value is an array. Search for corresponding key
                    if (isset($validatorRule[self::DEFAULT_VALUE][$key])) {
                        ...
                        // ZAP value array is provided, but it doesn't have an entry for current field
