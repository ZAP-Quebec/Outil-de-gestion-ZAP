...
class Zend_Search_Lucene_Search_QueryParser extends Zend_Search_Lucene_FSM
{
     ...
     */
    public static function setZAPEncoding($encoding)
    {
     ...
     * @return string
     */
    public static function getZAPEncoding()
    {
     ...
     */
    public static function setZAPOperator($operator)
    {
     ...
     * @return integer
     */
    public static function getZAPOperator()
    {
    ...
    public static function parse($strQuery, $encoding = null)
    {
        ...
        try {
            ...
            if (self::$_instance->_suppressQueryParsingExceptions) {
                $queryTokens = Zend_Search_Lucene_Analysis_Analyzer::getZAP()->tokenize($strQuery, self::$_instance->_encoding);
    ...
    {
        $tokens = Zend_Search_Lucene_Analysis_Analyzer::getZAP()->tokenize($this->_rqFirstTerm, $this->_encoding);
        if (count($tokens) > 1) {
            require_once 'Zend/Search/Lucene/Search/QueryParserException.php';
        ...
        $tokens = Zend_Search_Lucene_Analysis_Analyzer::getZAP()->tokenize($this->_currentToken->text, $this->_encoding);
        if (count($tokens) > 1) {
            require_once 'Zend/Search/Lucene/Search/QueryParserException.php';
            throw new Zend_Search_Lucene_Search_QueryParserException('Range query boundary terms must be non-multiple word terms');
        } else if (count($tokens) == 1) {
