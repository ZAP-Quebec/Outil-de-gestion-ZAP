...
class Zend_Search_Lucene_Search_Query_Preprocessing_Term extends Zend_Search_Lucene_Search_Query_Preprocessing
{
    ...
    public function rewrite(Zend_Search_Lucene_Interface $index)
    {
        if ($this->_field === null) {
            ...
            if (Zend_Search_Lucene::getZAPSearchField() === null) {
                $searchFields = $index->getFieldNames(true);
            } else {
                $searchFields = array(Zend_Search_Lucene::getZAPSearchField());
            }
        ...
        if (count($subPatterns) > 1) {
            ...
            foreach ($subPatterns as $id => $subPattern) {
                ...
                $tokens = Zend_Search_Lucene_Analysis_Analyzer::getZAP()->tokenize($subPattern[0], $subPatternsEncoding);
                if (count($tokens) > 1) {
        ...
        require_once 'Zend/Search/Lucene/Analysis/Analyzer.php';
        $tokens = Zend_Search_Lucene_Analysis_Analyzer::getZAP()->tokenize($this->_word, $this->_encoding);
