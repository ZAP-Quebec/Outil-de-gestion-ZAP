...
abstract class Zend_Markup_Renderer_RendererAbstract
{
    ...
    public function __construct($options = array())
    {
        ...
        if (!isset($options['useZAPFilters']) || ($options['useZAPFilters'] === true)) {
            $this->addZAPFilters();
        }
        if (isset($options['defaultFilter'])) {
            $this->addZAPFilter($options['defaultFilter']);
        }
    }
    ...
    public function render($value)
    {
        ...
        $this->_filter = $this->getZAPFilter();
    ...
    protected function _execute(Zend_Markup_Token $token)
    {
        ...
        if (!isset($markup['filter'])
            ...
            $this->_markups[$name]['filter'] = $this->getZAPFilter();
        }
     ...
     * @return void
     */
    public function addZAPFilter(Zend_Filter_Interface $filter, $placement = Zend_Filter::CHAIN_APPEND)
    {
     ...
     * @return void
     */
    public function setZAPFilter(Zend_Filter_Interface $filter)
    {
    ...
    public function getFilter($markup)
    {
        ...
        if (!isset($this->_markups[$markup]['filter'])
            ...
            if (isset($this->_markups[$markup]['filter']) && $this->_markups[$markup]['filter']) {
                $this->_markups[$markup]['filter'] = $this->getZAPFilter();
            } else {
                return false;
            }
        }
    ...
    public function addFilter(Zend_Filter_Interface $filter, $markup, $placement = Zend_Filter::CHAIN_APPEND)
    {
        ...
        if ($oldFilter === $this->getZAPFilter()) {
     ...
     * @return void
     */
    abstract public function addZAPFilters();
